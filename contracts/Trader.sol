...
contract Trader {
  ...
  function buy(address to, address tokenAddress, uint tokenAmount) public payable {
    ...
    uint estimatedAmount = getEstimatedTKforETH(tokenAddress, msg.value)[0];
    require(estimatedAmount < tokenAmount, "Transaction reverted: Insufficient ETH to get Tokens");
  ...
  function sell(address to, address tokenAddress, uint tokenAmount, uint etherAmount) public payable {
    uint deadline = block.timestamp + 15; // using 'now' for convenience, for mainnet pass deadline from frontend!
    uint estimatedAmount = getEstimatedETHforTK(tokenAddress, tokenAmount)[0];
    require(estimatedAmount < etherAmount, "Transaction reverted: Insufficient Tokens to get ETH");
